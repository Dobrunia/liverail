...
- [x] **Event reducer / applier registration**
...
## 10B. Runtime Introspection, Lifecycle and Operational Safety
...
- [ ] **Runtime contract introspection**  
       ...
       **Какие части затрагивает:** `contracts`, `server`, частично `client`  
       **Архитектурные нюансы:** это должен быть безопасный read-only слой поверх registry, а не обход внутренних структур напрямую. Нельзя допустить, чтобы introspection API позволяло менять registry.  
       **Не забыть обновить:** contract registry model, strongly typed public API, а также diagnostics later, если они будут использовать этот introspection layer.  
       **Почему здесь:** это первый шаг в сторону operational/debug-фич, и он нужен раньше остальных задач этого блока.
...
- [ ] **Runtime health and debug utilities**  
       ...
       **Какие части затрагивает:** `server`, частично `client`  
       **Архитектурные нюансы:** утилиты должны быть read-only и не должны влиять на поведение production runtime. Лучше строить их поверх runtime contract introspection и channel membership runtime, а не дублировать источники данных.  
       **Не забыть обновить:** runtime contract introspection, channel membership runtime, public API, если debug utilities выходят наружу как официальный API.  
       **Почему здесь:** после появления introspection можно строить человеко-понятные debug utilities.
...
- [ ] **Client connection lifecycle model**  
       ...
       **Какие части затрагивает:** `client`  
       **Архитектурные нюансы:** lifecycle state должен быть одной централизованной моделью, а не набором флагов. Все transport-specific переходы должны приводиться к этой общей модели.  
       **Не забыть обновить:** reconnect-safe resubscription, unified realtime error model, typed public API, diagnostics, если они показывают состояние клиента.  
       **Почему здесь:** это первая важная operational-фича на клиенте после базового runtime и reconnect.
...
- [ ] **Server lifecycle hooks**  
       ...
       **Какие части затрагивает:** `server`  
       **Архитектурные нюансы:** hooks должны быть встроены в runtime lifecycle и вызываться детерминированно. Не стоит превращать их в полноценную plugin system.  
       **Не забыть обновить:** channel membership runtime, connection/join policy enforcement, unified error model, если lifecycle hooks могут репортить ошибки или failures.  
       **Почему здесь:** после client lifecycle логично закрыть lifecycle и на сервере.
...
- [ ] **System event model**  
       ...
       **Какие части затрагивает:** `contracts`, `client`, частично `server`  
       **Архитектурные нюансы:** системные события должны быть отделены от пользовательских domain events. Они не должны смешиваться с обычными `event(...)` без явной маркировки.  
       **Не забыть обновить:** unified realtime error model, client connection lifecycle model, server lifecycle hooks, typed event listener API, docs later.  
       **Почему здесь:** после явной lifecycle-модели нужен строгий способ сообщать об этих состояниях наружу.
...
- [ ] **Channel instance utilities**  
       ...
       **Какие части затрагивает:** `contracts`, `server`, `client`  
       **Архитектурные нюансы:** utilities должны опираться на channel contracts и key contracts, а не на самодельные строковые идентификаторы. Важно зафиксировать один канонический способ сериализации.  
       **Не забыть обновить:** channel key contracts, channel membership runtime, typed channel subscription API, transport adapters, если они используют сериализованные channel ids.  
       **Почему здесь:** это естественное усиление уже существующей channel-модели.
...
- [ ] **Unsubscribe and cleanup guarantees**  
       ...
       **Какие части затрагивает:** `client`, `server`  
